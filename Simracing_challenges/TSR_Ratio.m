...
% for idx = 1:1
    ...
    % Ratio, F/R
        s.(['DIFF_TSR_' run]) = s.(['AVE_TSR_F_' run]) - s.(['AVE_TSR_R_' run]);
...
%             end
%         end
        plot(s.(['Lap_Dist_' run]), movmean(s.(['DIFF_TSR_' run]), 25), 'LineStyle', style_plot{mod(idx,2)+1}, 'LineWidth', 1.5, 'Color', color{idx})%, 'Marker', markers(idx))
        hold on
...
set(x, 'FontSize', 30)
y = ylabel('Diffierence in average axle slip ratios, F/R', 'FontName', 'Serif');
set(y, 'FontSize', 30)
