...
manual_legend = {'Lamborghini R16', 'Mercedes R13', 'Porsche R14'};
...
%         patch(s.(['Lap_Dist_' run]), s.(['DIFF_TSA_' run]),s.(['Speed_' run]), 'FaceColor', 'none', 'EdgeColor', 'interp', 'Marker', markers(idx), 'LineWidth', 1.5)
        ...
        plot(s.(['Lap_Dist_' run]), s.(['Speed_' run]),'LineStyle', '-', 'LineWidth', 1.5, 'Color', color{idx})
        hold on
