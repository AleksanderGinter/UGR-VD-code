...
folder = 'D:\UGRacing\UGR EV24\SimRacing\Challenge4\matlab\aero_bal_merc';

manual_legend = {'Mercedes R8', 'Mercedes R9 | -2 RW', 'Mercedes R10 | -1 RW',};
% 'Lamborghini R10', 'Lamborghini R11 | -2 RW', 'Lamborghini R12 | -1 RW', ...
...
%         patch(s.(['Lap_Dist_' run]), s.(['DIFF_TSA_' run]),s.(['Speed_' run]), 'FaceColor', 'none', 'EdgeColor', 'interp', 'Marker', markers(idx), 'LineWidth', 1.5)
        ...
        plot(s.(['Lap_Dist_' run]), s.(['Speed_' run]),'LineStyle', style{idx}, 'LineWidth', 1.5, 'Color', color{idx})
        hold on
