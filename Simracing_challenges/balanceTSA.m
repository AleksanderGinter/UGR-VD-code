...
folder = 'D:\UGRacing\UGR EV24\SimRacing\Challenge4\matlab\diff_power';
...
%         scatter(s.(['TSA_FL_Time_' run]), s.(['DIFF_TSA_' run]),18, s.(['speed_' run]), markers(idx));
        ...
        plot(s.(['Lap_Dist_' run]), s.(['DIFF_TSA_' run]), 'LineStyle', style{mod(idx,2)+1}, 'LineWidth', 1.5, 'Color', color{idx})%, 'Marker', markers(idx))
        hold on
